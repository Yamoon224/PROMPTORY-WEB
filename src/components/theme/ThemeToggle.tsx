...
export function ThemeToggle({ showLabels = false, className }: { showLabels?: boolean; className?: string }) {
  ...
  return (
    <div
      ...
      className={cn(
        "inline-flex items-center gap-0.5 rounded-2xl border border-[var(--hairline)] bg-[var(--surface)] p-0.5",
        className,
      )}
    >
      {OPTIONS.map(({ value, label, Icon }) => {
        ...
        return (
          <button
            ...
            className={cn(
              "inline-flex items-center gap-1.5 rounded-2xl px-2 py-1.5 text-xs font-semibold transition-colors",
              isActive
